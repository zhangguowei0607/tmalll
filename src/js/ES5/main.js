...
window.onscroll = function () {
  var oDiv = document.querySelector('.menu');
...
$("#LoutiNav ul li").not(".last").click(function () {
  ...
  $(this).find("span").addClass("active_a"); //点击左边导航 然后跳到指定的楼层
...
$(window).scroll(function () {
  if (mark == 1) {
    ...
    var $obj = $("#main .Louti"); //循环每一个Louti 然后找到最先满足条件的那个 Louti

    $obj.each(function () {
      var $index = $(this).index(); //楼层与浏览器上面的高度

      var $height = $obj.eq($index).offset().top + $(this).height() / 2; //alert($height) 
